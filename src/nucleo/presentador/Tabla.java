...
public final class Tabla {
    ...
    private Bloque initialTableBlock;
    ...
    public Tabla(Tablero board, int tableWidth, List<String> headersList, List<List<String>> rowsList) {
        this.board = board;
        if (tableWidth <= 0) {
        ...
        for (String cell : firstRow) {
            ...
            try {
                Long.parseLong(cell);
                alignMode = Bloque.DATA_MIDDLE_RIGHT;
                ...
                try {
                    ...
                    try {
                        ...
                        alignMode = Bloque.DATA_MIDDLE_LEFT;
                    }
                }
            }
    ...
    public Tabla(Tablero board, int tableWidth, List<String> headersList, List<List<String>> rowsList, List<Integer> colWidthsList) {
    ...
    public Tabla(Tablero board, int tableWidth, List<String> headersList, List<List<String>> rowsList, List<Integer> colWidthsList, List<Integer> colAlignsList) {
    ...
    public Tabla setHeadersList(List<String> headersList) {
        this.headersList = headersList;
        return this;
    }
    ...
    public Tabla setRowsList(List<List<String>> rowsList) {
        this.rowsList = rowsList;
        return this;
    }
    ...
    public Tabla setColWidthsList(List<Integer> colWidthsList) {
        if (colWidthsList.size() != headersList.size()) {
            throw new RuntimeException("Column width count(" + colWidthsList.size() + ") and header size(" + headersList.size() + ") are not equal");
        } else {
            this.colWidthsList = colWidthsList;
    ...
    public Tabla setColAlignsList(List<Integer> colAlignsList) {
        if (colAlignsList.size() != headersList.size()) {
            throw new RuntimeException("Column align count(" + colAlignsList.size() + ") and header size(" + headersList.size() + ") are not equal");
        } else {
            this.colAlignsList = colAlignsList;
    ...
    public Tabla setHeaderHeight(int headerHeight) {
    ...
    public Tabla setRowHeight(int rowHeight) {
    ...
    public Tabla setGridMode(int gridMode) {
    ...
    public Bloque tableToBlocks() {
        for (int i = 0; i < headersList.size(); i++) {
            String headerValue = headersList.get(i);
            int columnWidth = colWidthsList.get(i);
            Bloque block = new Bloque(board, columnWidth, headerHeight, headerValue);
        ...
        if (getGridMode() != GRID_COLUMN) {
            for (int i = 0; i < rowsList.size(); i++) {
                List<String> row = rowsList.get(i);
                Bloque rowStartingBlock = initialTableBlock.getMostBelowBlock();
                for (int j = 0; j < row.size(); j++) {
                    String rowValue = row.get(j);
                    int columnWidth = colWidthsList.get(j);
                    Bloque block = new Bloque(board, columnWidth, rowHeight, rowValue);
                    if (getGridMode() == GRID_NON) {
                        block.allowGrid(false);
                    } else {
                        block.allowGrid(true);
            ...
            for (int i = 0; i < headersList.size(); i++) {
                ...
                Bloque block = new Bloque(board, colWidthsList.get(i), rowsList.size(),columnData);
                int alignIndex = colAlignsList.get(i);
                    block.setDataAlign(alignIndex);
